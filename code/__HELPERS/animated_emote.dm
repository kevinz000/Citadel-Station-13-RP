...
/proc/prepare_animated_pose_parts(mob/living/carbon/human/H)
	...
	head.filters += filter("alpha", 0, 0, icon(maskfile, "mask_head"))
	body.filters += filter("alpha", 0, 0, icon(maskfile, "mask_body"))
	larm.filters += filter("alpha", 0, 0, icon(maskfile, "mask_larm"))
	rarm.filters += filter("alpha", 0, 0, icon(maskfile, "mask_rarm"))
	lleg.filters += filter("alpha", 0, 0, icon(maskfile, "mask_lleg"))
	rleg.filters += filter("alpha", 0, 0, icon(maskfile, "mask_rleg"))
	return returned

/proc/t_pose(mob/living/carbon/human/H)
	if(H.dir & (EAST|WEST))
		return
	var/list/obj/effect/overlay/effects = prepare_animateed_pose_parts(H)
	// we want to hide their arms
	var/static/icon/maskfile = 'icons/effects/animated_emotes.dmi'
	var/filter/flarm = filter("alpha", 0, 0, icon(maskfile, "mask_larm"))
	var/filter/frarm = filter("alpha", 0, 0, icon(maskfile, "mask_rarm"))
	H.filters += flarm
	H.filters -= frarm
	// 10 ds total
